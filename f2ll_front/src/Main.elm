...
type alias Model = {fortran : String, llvm : String, result: String}
type alias ResponseS = { llvm : String, result : String}

init = (Model "" "" "", Cmd.none)

-- UPDATE

type Msg = NewContent String | Send | IpAddr (Result Http.Error ResponseS)
...
--                    div [ class "align-right", myStyle] [ Html.p [] [text model.llvm]]
                    ]
                ]
        ]
    ]
...
getRandomGif str =
  let
    url = "http://localhost:8080/api/main/run"
  in
    Http.send IpAddr (Http.post url (body str) decodeResult)

body str = Http.jsonBody (Encode.object [("fortranCode", Encode.string str)])

decodeResult = Decode.map2 ResponseS
        (Decode.at ["llvm"] Decode.string)
        (Decode.at ["result"] Decode.string)

statusStyle str =
    let
        margin_style = ("margin-right", "20px")
    in
    case str of
        "Ok" -> style [("color", "green"), margin_style]
        "" -> style [margin_style]
        _ -> style [("color", "red"), margin_style]
